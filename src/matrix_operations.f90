...
!------------------------------------------------------------------------
      ...
      !       (mu_phi .eqv. .true.) .and. (rheology .eq. 4)) then
        ...
        elseif ((dilatancy .eqv. .true.) .and. (correction .eqv. .true.) .and. &
            ...
            !------------------------------------------------------------------------
            !     d [  zeta  du/dx ] / dx      B1_4, D1_4 
            !------------------------------------------------------------------------

              Ax(k) = Ax(k) + &
                  utp(i,j) * ( zetaC(i,j) + zetaC(i-1,j)) / Deltax2   - &
                  (zetaC(i,j)   * utp(i+1,j) &
...
!------------------------------------------------------------------------
           ...
           !---- FOR MU(I) RHEOLOGY ----!
            ...
            elseif ((dilatancy .eqv. .true.) .and. (correction .eqv. .true.) .and. &
                ...
                !------------------------------------------------------------------------
                !     d [ zeta dv/dy ] / dy   D2_4, B2_4
                !------------------------------------------------------------------------

                Ax(k) = Ax(k) + &
                    vtp(i,j) * (zetaC(i,j) + zetaC(i,j-1)) / Deltax2 - &
                    ( zetaC(i,j)   * vtp(i,j+1) &  
