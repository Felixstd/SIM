...
"""
Almost all of this code was taken from Nills Hutter.

See Hutter, 2019 Leads and ridges in Arctic sea ice from RGPS data and a new tracking algorithm

His technique is applied to simulation data to find the angle of intersection between 2 lkfs. 
...
def segs_intersection(seg_I, seg_II):
    """
    This function is used to compute the intersection between two line segments.
    https://en.wikipedia.org/wiki/Line–line_intersection.
    

    Args:
        seg_I (_type_): _description_
        seg_II (_type_): _description_
    """
    
    point_start_I = seg_I[:,0]
    point_start_II = seg_II[:,0]
    
    point_end_I = seg_I[:,-1]
    point_end_II = seg_II[:,-1]
    ...
    x_3 = point_start_II[0]
    y_3 = point_start_II[1]
    
    x_4 = point_end_II[0]
    y_4 = point_end_II[1]
    
    
    t = ((x_1 - x_3)*(y_3 - y_4) - (y_1 - y_3)*(x_3 - x_4)) / \
        ((x_1 - x_2) * (y_3 - y_4) - (y_1 - y_2) * (x_3 - x_4))
        
    u = - ((x_1 - x_2)*(y_1 - y_3) - (y_1 - y_2)*(x_1 - x_3)) / \
        ((x_1 - x_2) * (y_3 - y_4) - (y_1 - y_2) * (x_3 - x_4))
...
def angle_segs_2(seg_I,seg_II, intersection):
    """ Function to compute the angle between two segments.
    ...
    # Determine directions of segments
    e1 = (seg_I[:,0]-intersection)
    e1 = (e1/np.sqrt(np.sum(e1**2))) # Normalize basis vector

    f1 = (seg_II[:,0]-intersection)
    f1 = (f1/np.sqrt(np.sum(f1**2))) # Normalize basis vector
...
def lkf_detection(eps_tot, max_kernel, min_kernel, dog_thres, dis_thres, \
    ellp_fac, angle_thres, eps_thres, lmin, max_ind, use_eps, skeleton_kernel, uniaxial = True) : 
    """
    This function takes as input the total deformation field and performs the algorithm
    of Hutter. 
    
    """

    #find the log of the deformation
    ...
    plt.pcolormesh(lkf_detect)
    plt.savefig('test_thin.png')
    ...
    if uniaxial:
        ...
        for seg in filtered_segs:
            for seg2 in filtered_segs:
                # plt.plot(seg[1,:]+0.5,seg[0,:]+0.5)
                # plt.plot(seg2[1,:]+0.5,seg2[0,:]+0.5)
                # plt.savefig('test2.png')
                
                #If both are not the same lkf
                if ~np.array_equal(seg, seg2):
                    Point_intersection = segs_intersection(seg, seg2)
                    
                    #if they intersect
                    if Point_intersection:
                        
                        angle = angle_segs_2(seg, seg2, Point_intersection)
                        
                        if angle > 90:
                            angle = 180 - angle
                        #compute the angles between the two
                        angle_lkfs.append(angle)
                        pairs_lkf.append([seg, seg2])
    
    return pairs_lkf, angle_lkfs
