...
def read_data_var(var, expno, Parameters, start, dates, outputdir, netcdf_file):
    
    Nx = Parameters.Nx
    Ny = Parameters.Ny   
    Ndates = len(dates)
    time = np.arange(0, Ndates)*Parameters.dt
    X = np.arange(0, Nx)
    Y = np.arange(0, Ny)
    ...
    for k, date in enumerate(dates, start=start):
        ...
        for prefix, key in files_info:
            ...
            data_list.append(np.genfromtxt(filename, dtype=None))
    ...
    ds = xr.Dataset(
        ...
        attrs={
            "description": f"Model output for {var} with expno {expno}",
