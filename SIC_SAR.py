...
def finding_interestSIC_point(latitudes, longitudes, polygon_radar) :
    ...
    # points = np.vstack(longitudes.flat, longitudes)
    ...
    for i in range(width) : 
        for j in range(height) : 
            ...
            contains = is_inside_sm(polygon_radar, (longitude, latitude))
...
def SIC_Modis(SICDir, ParamsDir, PlotDir, latitudes, longitudes, idx_interest_SIC, polygon, lon_0, lat_ts, lon_utq, lat_utq, plotting = False) :
    ...
    for dir in os.listdir(SICDir) : 
        ...
        if (int(dir) == 2022) or (int(dir) == 2023) : 
            ...
            SIC_min_year = []
            SIC_max_year = []
            for month in sorted(os.listdir(directory_SIC)) : 
                
                for file in sorted(os.listdir(directory_SIC+month)):
                    ...
                    SIC_Arctic_Day = nc.Dataset(directory_SIC+month+'/'+file, mode = 'r')
            ...
            np.save(ParamsDir+str(dir)+'_MODIS_rightlatlon.npy', dict_ICE)
...
def SIC_CDR(SICDir, ParamsDir, PlotDir, latitudes, longitudes, idx_interest_SIC, polygon, lon_0, lat_ts, lon_utq, lat_utq, plotting = False) : 
    ...
    for dir in os.listdir(SICDir) : 
        ...
        if (dir == 2022) or (dir == 2023) : 
            ...
            np.save(ParamsDir+str(dir)+'_CDR_rightlatlon.npy', dict_ICE)
...
kml_file = 'radius-around-bank_utq.kml'
kml_file_CDR = 'rad_around_utq_CDR_3.kml'


# lat_utq, lon_utq = 71.319770, -156.706460

lat_utq = 71 + 17/60 + 31/3600
lon_utq = -(156 + 47/60 + 7/3600)

# 71°17'31"N 156°47'07"W

# 71°17'32.5"N 156°47'16.3"W
...
# print(latitudes_field_view, longitudes_field_view)
...
polygon_radar_fieldview_mod = np.asarray(polygon_radar_fieldview.exterior.coords)
polygon_radar_fieldview_CDR = np.asarray(polygon_radar_fieldview_CDR.exterior.coords)
...
point_MODIS = 0
if point_MODIS:
    MOD = np.flipud(nc.Dataset(SICDir_MODIS+'2022/12/'+'sic_modis-aqua_amsr2-gcom-w1_merged_nh_1000m_20221221.nc', mode = 'r')['sic_merged'][:])
    idx_point_SIC = finding_interestSIC_point(latitudes, longitudes, polygon_radar_fieldview_mod)
    ...
    np.save('idx_polygon_MODIS_rightLatlon.npy', point)
# 
idx_point_SIC = np.load(ParamsDir_MODIS+'idx_polygon_MODIS_rightLatlon.npy', allow_pickle=True).item()['idx_points']
# # # print(idx_point_SIC)
...
point_interest = 0
if point_interest:
    idx_i = np.arange(3241, 3262)
    idx_j = np.arange(747, 768)
    ...
    np.save(ParamsDir_MODIS+'idx_polygon_MODIS_BankLatlon.npy', point)
...
if point_CDR:
    ...
    idx_point_SIC_CDR = np.delete(idx_point_SIC_CDR, [1,1], axis=0)
    idx_point_SIC_CDR = np.delete(idx_point_SIC_CDR, [2,2], axis=0)
    idx_point_SIC_CDR = np.delete(idx_point_SIC_CDR, [1], axis=0)
    # idx_point_SIC_CDR = np.delete(idx_point_SIC_CDR, [0,1], axis=0)
    point = {'Idx' : idx_point_SIC_CDR}
    np.save(ParamsDir_CDR+'idx_polygon_CDR_rightlatlon.npy', point)

idx_point_SIC_CDR = np.load(ParamsDir_CDR+'idx_polygon_CDR_rightlatlon.npy', allow_pickle=True).item()['Idx']
# 
# SIC_CDR = np.flipud(nc.Dataset(SICDir_CDR+'2022/seaice_conc_daily_nh_20220629_f17_v04r00.nc', mode = 'r')['cdr_seaice_conc'][:])   
# # print(SIC_CDR)
# print(SIC_CDR[0][idx_point_SIC_CDR[:, 0], idx_point_SIC_CDR[:, 1]])
...
# SIC_MOD = np.flipud(nc.Dataset(SICDir_MODIS+'2022/12/'+'sic_modis-aqua_amsr2-gcom-w1_merged_nh_1000m_20221221.nc', mode = 'r')['sic_merged'][:])   
